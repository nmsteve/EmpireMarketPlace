...
contract EmpireCollection is ERC721, Ownable, ERC721Royalty, IEmpireCollection{
    ...
    constructor(string memory baseURI_, string memory name, string memory ticker) ERC721(name, ticker) {
        // Set contract URI
        baseURI = baseURI_;
    ...
    function mint(uint256 num, address token, uint256 mintPriceInWei, address user, uint96 royalties) override public onlyOwner{
    ...
    function mintBNB(uint256 num, uint256 mintPriceInWei, address user, uint96 royalties) public override payable onlyOwner{
        uint256 amount = num * mintPriceInWei;
...
contract CollectionMinter is Ownable {
    ...
    function createNewCollection(string memory name, string memory symbol, string memory _baseURI) external{
        _collectionIds.increment();
        uint256 newCollectionId = _collectionIds.current();
        EmpireCollection collection = new EmpireCollection(_baseURI,name,symbol);
